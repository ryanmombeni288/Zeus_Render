	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/test-proxy" && request.method === "POST") {
			...
			try {
				...
				let resStr = "";
				const dec = new TextDecoder();
				const timeoutId = setTimeout(() => { try { s.close(); } catch(e){} }, 3000);
				try {
					while (true) {
						const res = await reader.read();
						if (res.done || !res.value) break;
...
const COMMON_WAVES_SCRIPT = `
    ...
    <script>
      (function initWaves(){
        ...
        const renderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: true, alpha: false, powerPreference: "default" });
        ...
        camera.position.set(0, 0, IS_MOBILE ? 28 : 14); 

        const segX = IS_MOBILE ? 35 : 80;
        const segY = IS_MOBILE ? 20 : 40;
        const geom = new THREE.PlaneGeometry(80, 40, segX, segY);
                ...
                tbody.innerHTML = users.map(user => {
                    ...
                    return '<tr class="group transition-all drop-shadow-sm bg-white/40 dark:bg-zinc-900/20" data-username="' + user.username + '">' +
                                '<td class="bg-white/60 dark:bg-zinc-900/40  group-hover:bg-white/80 dark:group-hover:bg-zinc-900/60 p-1 rounded-r-md border-y border-r border-gray-200 dark:border-zinc-800 text-center select-none">' +
                                    ...
                                    '</div>' +
                                '</td>' +
                                '<td class="bg-white/60 dark:bg-zinc-900/40  group-hover:bg-white/80 dark:group-hover:bg-zinc-900/60 p-1 border-y border-gray-200 dark:border-zinc-800 text-center">' +
                                ...
                                '<td class="bg-white/60 dark:bg-zinc-900/40  group-hover:bg-white/80 dark:group-hover:bg-zinc-900/60 p-1.5 border-y border-gray-200 dark:border-zinc-800 text-center">' +
                                    ...
                                    '</div>' +
                                '</td>' +
                                '<td class="bg-white/60 dark:bg-zinc-900/40  group-hover:bg-white/80 dark:group-hover:bg-zinc-900/60 p-1 border-y border-gray-200 dark:border-zinc-800">' +
